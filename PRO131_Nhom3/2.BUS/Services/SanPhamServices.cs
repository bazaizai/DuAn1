...
namespace _2.BUS.Services
{
    public class SanPhamServices:ISanPhamServices
    {
        ...
        private string MaTT()
        {
            if (_iSanPhamRepository.GetSanPhams().Count > 0)
            {
                return "SP" + Convert.ToString(_iSanPhamRepository.GetSanPhams().Max(c => Convert.ToInt32(c.Ma.Substring(2, c.Ma.Length - 2)) + 1));
            }
            else return "SP1";
        }
    }
