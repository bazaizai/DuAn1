...
namespace _3.PL.Views
{
    ...
    {
        private IHinhThucMhServices _ISanPhamServices;
        private List<HinhThucMhViews> _LstSanPham;
        public HinhThucMhViews? _SanPham;
        public FrmHinhThucMh()
        {
            InitializeComponent();
            _ISanPhamServices = new HinhThucMhServices();
            _LstSanPham = new List<HinhThucMhViews >();
            tb_Ma.Enabled = false;
        }
        public void LoadData()
        {
            ...
            tb_Ma.Enabled = false;
            var lstKhachHang = _ISanPhamServices.GetHinhThucMhs();
            if (tb_TimKiem.Text != "")
            {
        ...
        private string MaTS()
        {
            ...
            int stt = _ISanPhamServices.GetHinhThucMhs().Where(x => Regex.Match(x.Ma, @"^[^0-9]*").Value == tenVT).ToList().Count + 1;
        ...
        private void dtg_Show_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                _SanPham = _ISanPhamServices.GetHinhThucMhs().FirstOrDefault(x => x.Id == Guid.Parse(r.Cells[0].Value.ToString() ?? "Unknown message id"));
